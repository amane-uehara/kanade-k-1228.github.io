...
$$
\psi\_{\bm{k}}(r)=A\exp(i\bm{k}\cdot \bm{r})
$$
...
$$
N(k) = \f{\f{4}{3}\pi k^3}{\f{1}{2} \l(\f{2\pi}{L}\r)^3}
$$

エネルギー $E$ 以下の電子の数 $N(E)$ は

$$
N(E) = \f{L^3}{3\pi^2}\l(\f{2mE}{\hbar^2}\r)^{\f{3}{2}}
$$

エネルギー $E$ にある電子の数密度を、状態密度 $D(E)$ とすると、

$$
D(E) = \d{}{E}\f{N(E)}{L^3} = \f{1}{2\pi^2}\l(\f{2m}{\hbar^2}\r)^{\f{3}{2}}E^{\f{1}{2}}
$$

☆ 状態密度はエネルギーの 1/2 乗に比例する

### 3.4. フェルミ準位

最外殻電子は、共有結合を作ったり、金属の自由電子となったり、その原子の特性に大きく影響する。ところで、最外殻電子は原子の中で最も高いエネルギー状態にある電子なので、k 空間上でも「最外殻」に位置している。
...
$$
E = 2 \sum\_{E<E_P} \f{\hbar^2|\bm{k}|^2}{2m} \sim \f{V}{\pi^2}\f{\hbar^2 k_F^5}{10m}
$$

### 3.5. 金属の電子比熱

温度 0 のとき、フェルミ準位を超えた電子はいないが、温度が上がると少しずつフェルミ準位を飛び出した電子がみられるようになる。（熱励起）

どの程度、フェルミ準位を超えて飛び出すかを表したのがフェルミ分布関数

$$
f(E,T) = \f{1}{\exp\l(\f{E-\mu}{k_BT}\r)+1}
$$

$\mu$ は化学ポテンシャル $\sim E_F$

ある温度の電子の状態分布は、状態密度 × フェルミ分布関数 で求まる。

$$
D(E,T) = D(E) \times f(E,T)
$$
...
$$
\Delta U = U(T) - U(0) \sim (k_B T)\l(n\f{T}{T_F}\r)
$$
...
$$
V(\bm{r})=\sum_n v(\bm{r}-\bm{R}_n)
$$

周期的なポテンシャルに対する、シュレディンガー方程式

$$
\l[-\f{\hbar^2}{2m}\nabla^2+V(\bm{r})\r]\phi(\bm{r})=E\phi(\bm{r})
$$

の解は、周期的なブロホ関数

$$
\phi(\bm{r})=u_\bm{R}(\bm{r})\exp(i\bm{R}\cdot\bm{r})
$$

となる。ただし、$u$ には周期性がある。

$$
u_\bm{R}(\bm{r})=u_\bm{R}(\bm{r}+\bm{R})
$$
...
$$
\hbar k + \hbar k_P = \hbar k'
$$

光子の運動量は小さいので、バンド図上で真上に移動する。（$k=k'$）

エネルギー保存

$$
E(k) + \hbar \omega_P = E'(k')
$$

この式を満たす遷移が可能なとき、遷移が起きる。
